...
- **Stock management**: `StockManager` with atomic `reserve_stock()`/`release_stock()` using `select_for_update()` to prevent overselling under concurrent requests. `StockReservation` model with TTL (default 15 min). `StockLog` for audit trail.
