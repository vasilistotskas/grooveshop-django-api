...
- **Greeklish search**: Handled at *index time* — searchable Greek fields are shadowed by canonical `*_greeklish` attributes plus `*_greeklish_alt` word-initial μπ/ντ/γκ → b/d/g variants (`search/transliteration.py`), and queries reach Meilisearch verbatim. Never rewrite queries into space-joined variants: Meilisearch's default `last` matching strategy only drops words from the end (the first word is effectively mandatory) and only the first 10 query words are considered. Conventions one substitution away from canonical (η→"h", υ→"y") ride on typo tolerance instead of extra variants — but remember a typo on a word's FIRST character costs two typos.
