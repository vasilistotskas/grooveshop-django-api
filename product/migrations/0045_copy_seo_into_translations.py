...
``Product, ProductCategory`` carried ``seo_title`` / ``seo_description`` /
...
``0044_seo_translated_fields`` renamed those columns to ``shared_seo_*`` and
added the real fields to the parler translations model; this moves
the values across, and ``0046_remove_shared_seo_fields`` drops the shared
columns.
...
back onto the shared row, once ``0046_remove_shared_seo_fields``'s reverse has restored
the columns.
"""
