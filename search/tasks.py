...
def update_click_scores() -> dict[str, int]:
    ...
    changed = {}
    for result_type, model, index_name in (
        ...
        changed[result_type] = _apply_click_scores(model, index_name, scores)
...
def _apply_click_scores(model, index_name: str, scores: dict[int, int]) -> int:
    ...
    if changed_ids:
        _push_click_scores_to_meili(model, index_name, changed_ids)
    return len(changed_ids)


def _push_click_scores_to_meili(
    model, index_name: str, master_ids: list[int]
) -> None:
