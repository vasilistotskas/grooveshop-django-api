...
def test_print_db_config():
    ...
    test_db_name = settings.DATABASES['default'].get('TEST', {}).get('NAME', 'NOT_SET')
    
    print(f"\n[WORKER: {worker}] Default Name: {db_name}")
    print(f"[WORKER: {worker}] Test Name: {test_db_name}")
    print(f"[WORKER: {worker}] DB Host: {settings.DATABASES['default'].get('HOST')}")
    print(f"[WORKER: {worker}] DB Port: {settings.DATABASES['default'].get('PORT')}")
