...
class OrderPaymentViewSetTestCase(TestCase):
    ...
    @mock.patch('order.models.order.Order.is_paid', new_callable=mock.PropertyMock)
    def test_process_payment_authenticated_user(self, mock_is_paid):
        ...
        with mock.patch('order.views.payment_view.PayWayService.process_payment') as mock_process_payment:
    ...
    def test_process_payment_anonymous_user(self, mock_is_paid):
        ...
        request = self.factory.post(
            ...
            {
                'pay_way_id': self.bank_transfer_pay_way.id,
                'payment_data': {}
            },
            format='json'
        )
        ...
        with mock.patch('order.views.payment_view.PayWayService.process_payment') as mock_process_payment:
            ...
            self.assertTrue(request.session.get(f'order_{self.anon_order.uuid}'))
    ...
    def test_process_payment_unauthorized_user(self, mock_is_paid):
        ...
        with mock.patch('order.views.payment_view.PayWayService.process_payment') as mock_process:
    ...
    def test_check_payment_status(self, mock_check_status):
        ...
        self.assertEqual(response.data['payment_status'], PaymentStatusEnum.COMPLETED)
        
    def test_check_payment_status_no_payment(self):
        view = OrderPaymentViewSet.as_view({'get': 'check_payment_status'})
        request = self.factory.get(f'/api/v1/order/{self.user_order.pk}/payment-status')
        force_authenticate(request, user=self.user)
        response = view(request, pk=self.user_order.pk)
    ...
    def test_refund_payment_admin_user(self, mock_is_paid, mock_refund):
        ...
        request = self.factory.post(f'/api/v1/order/{self.paid_order.pk}/refund')
        force_authenticate(request, user=self.admin_user)
        response = view(request, pk=self.paid_order.pk)
