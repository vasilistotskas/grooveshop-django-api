...
class PayWayServiceTestCase(TestCase):
    ...
    def test_get_provider_for_pay_way(self, mock_get_payment_provider):
        ...
        mock_get_payment_provider.assert_called_once_with(self.online_pay_way.provider_code)
        
        mock_get_payment_provider.reset_mock()
        provider = PayWayService.get_provider_for_pay_way(self.offline_pay_way_with_confirmation)
        self.assertIsNone(provider)
        mock_get_payment_provider.assert_not_called()
        
    @mock.patch('pay_way.services.PayWayService.get_provider_for_pay_way')
    def test_process_offline_payment_without_confirmation(self, mock_get_provider):
        success, data = PayWayService.process_payment(
            pay_way=self.offline_pay_way_without_confirmation,
            order=self.order
        )
    ...
    def test_process_offline_payment_with_confirmation(self, mock_get_provider):
        success, data = PayWayService.process_payment(
            pay_way=self.offline_pay_way_with_confirmation,
            order=self.order
        )
    ...
    def test_refund_payment_online(self, mock_get_provider):
        ...
        self.order.mark_as_paid(payment_id="TEST_PAYMENT_123", payment_method="Stripe")
        self.order.save()
    ...
    def test_refund_payment_offline(self):
        self.order.payment_id = f"OFFLINE_{self.order.id}"
        self.order.payment_status = PaymentStatusEnum.COMPLETED
        self.order.mark_as_paid(payment_id=f"OFFLINE_{self.order.id}", payment_method="Bank Transfer")
        self.order.save()
